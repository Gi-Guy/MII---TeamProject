interface User {
  ...
  function renderUsers() {
    const tbody = document.querySelector<HTMLTableSectionElement>("#users-table tbody");
    if (!tbody) return;
    tbody.innerHTML = ''; 
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll<HTMLButtonElement>('.save-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        const roleSelect = document.querySelector(`select.role-select[data-userid="${userId}"]`) as HTMLSelectElement;
        const readCheckbox = document.querySelector(`input.perm-read[data-userid="${userId}"]`) as HTMLInputElement;
        const writeCheckbox = document.querySelector(`input.perm-write[data-userid="${userId}"]`) as HTMLInputElement;
        ...
        if (user) {
          ...
          user.permissions = [];
          if (readCheckbox.checked) user.permissions.push('read');
          if (writeCheckbox.checked) user.permissions.push('write');
